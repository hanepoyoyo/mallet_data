...
def create_model(corpus_dir, model_dir):
    ...
    mallet_modeling = f'mallet\\bin\\mallet train-topics --input "{corpus_path}" --num-topics 10 --output-state "{state_path}" --output-topic-keys "{keys_path}" --output-doc-topics "{topics_path}"'
